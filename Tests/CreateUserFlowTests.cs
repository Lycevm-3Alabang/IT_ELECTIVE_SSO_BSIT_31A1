...
public class CreateUserFlowTests
{
    ...
    {
        var userManagerMock = MockUserManager();
        userManagerMock.Setup(m => m.FindByEmailAsync("student@example.com")).ReturnsAsync((ApplicationUser?)null);
        userManagerMock.Setup(m => m.CreateAsync(It.IsAny<ApplicationUser>(), "Passw0rd!")).ReturnsAsync(IdentityResult.Success);
